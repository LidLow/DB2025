...
--Task 3.1
SELECT UPPER(first_name || ' ' || last_name), LENGTH(last_name), SUBSTRING(email FROM 1 FOR 3)
    FROM employees;

--Task 3.2
SELECT salary, ROUND(salary / 12, 2), salary * 0.1
    FROM employees;
...
--Task 4.2
SELECT project_name, (end_date - start_date) * 3600 AS hours_spend
    FROM projects;
...
SELECT employee_id, (first_name || ' ' || last_name) AS full_name, salary
    FROM employees
    WHERE salary > 65000
    UNION SELECT employee_id, (first_name || ' ' || last_name) AS full_name, salary
        FROM employees
        WHERE hire_date > '2020-01-01';
